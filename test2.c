...
#define ONVIF_PASSWORD "admin123"
...
void UserGetUri(struct soap *soap, struct _trt__GetProfilesResponse *trt__GetProfilesResponse,struct _tds__GetCapabilitiesResponse *capa_resp,char *token);
int UserSetSystemTime(struct soap *soap,struct _tds__GetCapabilitiesResponse *capa_resp);
        ...
        {  
			...
			for (;pcount < trt__GetProfilesResponse.__sizeProfiles ;pcount++){
				...
				if(trt__GetProfilesResponse.Profiles[pcount].VideoEncoderConfiguration->RateControl != NULL){
		     	...
		     	UserGetUri(soap, &trt__GetProfilesResponse, capa_resp,trt__GetProfilesResponse.Profiles[pcount].token);
			}
        }else{  
            printf("Profiles Get inner Error\n");  
        }  
        printf("Profiles Get Procedure over\n");
		UserSetSystemTime(soap,capa_resp);
    }
}  
...
int UserSetSystemTime(struct soap *soap,struct _tds__GetCapabilitiesResponse *capa_resp)
{
	...
	//tds__SetSystemDateAndTime->TimeZone->TZ = "CST-8:00:00";
	strncpy(tds__SetSystemDateAndTime->TimeZone->TZ,"UTC+0",strlen("UTC+0"));
	tds__SetSystemDateAndTime->DateTimeType = 0 ;
	...
	tds__SetSystemDateAndTime->UTCDateTime->Date->Month = tt->tm_mon;
	...
	if (ret == -1){
		if(soap->error){
			printf("soap_call___tds__SetSystemDateAndTime faild %d ,%s ,%s\n",soap->error, *soap_faultcode(soap), *soap_faultstring(soap));
			ret = soap->error;
		}
	}else{
		printf("soap_call___tds__SetSystemDateAndTime OK !!!!!!!!!\n");
	}
	return soap->error;
}
